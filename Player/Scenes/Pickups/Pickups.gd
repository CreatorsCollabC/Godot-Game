...
@export var spice : int: set = set_spice
...
func _ready() -> void:
	...
	spice_respawn_timer.connect("timeout", Callable(self, "spawn_spice"))
	add_child(spice_respawn_timer)
	spawn_spice()

func set_spice(p_spice : int):
	spice = p_spice
	spice_name = SpicePowerup.SpicePowerupEnum.keys()[spice].to_lower()
