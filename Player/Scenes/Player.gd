...
func _ready() -> void:
	for spice in  SpicePowerup.SpicePowerupEnum.values() :
		var instanced_scene = spice_powerup_scene.instantiate()
		instanced_scene.spice = spice
		instanced_scene.connect("use_spice", Callable(self, "use_spice").bind(spice))
		instanced_scene.connect("end_spice", Callable(self, "end_spice").bind(spice))
	...
	#Health
	var health_bar1 = health_scene.instantiate()
...
func _physics_process(_delta): 
	...
	move_and_slide()
	var new_velocity = velocity #able to move
...
func use_spice(spice : int) : #called when a spice efects starts
	match spice :
		SpicePowerup.SpicePowerupEnum.BLACK_PEPPER : #makes the player larger
			var tween := create_tween()
# warning-ignore:return_value_discarded
			tween.tween_property(self,"scale",size_scale_factor * Vector2.ONE,0.2)

		SpicePowerup.SpicePowerupEnum.GREEN_CHILLI: #makes the player faster
			acceleration *= acceleration_scale_factor
			max_speed *= max_speed_scale_factor

		SpicePowerup.SpicePowerupEnum.RED_CHILLI: # makes the player jump higher
			jump_power *= jump_power_scale_factor
			
		SpicePowerup.SpicePowerupEnum.GHOST_CHILLI:
			baseGravity = gravity
			$AnimationSprite.get_material().set_shader_parameter("effect",1)
			gravity = 0
			velocity.y = 0
...
func end_spice(spice : int) : #called when a spice effects ends
	match spice :
		SpicePowerup.SpicePowerupEnum.BLACK_PEPPER : #return player to its normal size
			var tween := create_tween()
# warning-ignore:return_value_discarded
			tween.tween_property(self,"scale", Vector2.ONE,0.2) 

		SpicePowerup.SpicePowerupEnum.GREEN_CHILLI: #return player to its normal speed
			acceleration /= acceleration_scale_factor
			max_speed /= max_speed_scale_factor

		SpicePowerup.SpicePowerupEnum.RED_CHILLI: # return player to its normal jump power
			jump_power /= jump_power_scale_factor
			
		SpicePowerup.SpicePowerupEnum.GHOST_CHILLI:
			$AnimationSprite.get_material().set_shader_parameter("effect",0)
			gravity = baseGravity
